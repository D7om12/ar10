...
def upload_tempimage(client):
     '''
         Upload a picture of a ariftten. We don't ship one, so get creative!
     '''
     config = {
         'album': album,
    ...
    try:
        ...
        if op.type == 32:
            if not op.param2 in Bots and admin:
              if not op.param2 in admin and creator:
        ...
        if op.type == 13:
            if op.param3 in mid:
                if op.param2 in mid:
                    G = mid.getGroup(op.param1)
                    G.preventJoinByTicket = False
                    mid.updateGroup(G)
                    Ticket = mid.reissueGroupTicket(op.param1)
                    arif.acceptGroupInvitationByTicket(op.param1,Ticket)
                    G.preventJoinByTicket = True
                    mid.updateGroup(G)
                    Ticket = mid.reissueGroupTicket(op.param1)
...
#----------------------------------------------
        ...
        #      #if msg.from_ in creator:
            ...
            elif "Wiarifpedia " in msg.text:
                  try:
                      wiarif = msg.text.lower().replace("Wiarifpedia ","")
                      wiarifpedia.set_lang("id")
                      pesan="Title ("
                      pesan+=wiarifpedia.page(wiarif).title
                      pesan+=")\n\n"
                      pesan+=wiarifpedia.summary(wiarif, sentences=1)
                      pesan+="\n"
                      pesan+=wiarifpedia.page(wiarif).url
		    ...
		    a = msg.text.replace("Xvideos ","")
                ...
                msg.contentMetadata = {'mid': mid}
                arif.sendMessage(msg)
                arif.sendText(msg.to,"AMAN TERKENDALI ")
            elif "B1" == msg.text:
              #if msg.from_ in creator + admin:
                msg.contentType = 13
                msg.contentMetadata = {'mid': mid}
...
#=====================================================================================   
                ...
                try:
                    ...
                    if strnum == "off":
                        ...
                        if wait["lang"] == "JP":
                            arif.sendText(msg.to,"Itu off undangan ditolak✖\nSilakan arifrim dengan menentukan jumlah orang ketika Anda menghidupkan✖")
                        else:
                ...
                if msg.toType == 1:
                    ...
                    turl = arif.getUserTicket(tid)
                    arif.sendText(msg.to,"line://ti/p" + turl)
                else:
                    arif.sendText(msg.to,"error")
...
#                    arif.sendText(msg.to,"Tidak dapat digunakan untuk kelompok selain")
              ...
              if msg.from_ in admin or creator:
                ...
                arif.sendText(msg.to,"already turn active")
            elif msg.text in ["Simisimi off","Simisimi:off"]:
                settings["simiSimi"][msg.to] = False
                arif.sendText(msg.to,"BOT SIMISIMI TURN OFF")
                arif.sendText(msg.to,"already non active")
	...
	#      if msg.toType == 2:
                ...
                if wait["kickMention"] == True:
                    if wait["lang"] == "JP":
                        arif.sendText(msg.to,"☠️DANGER TAG KICK ON☠️")
                else:
                ...
                if wait["kickMention"] == False:
                   if wait["lang"] == "JP":
                        arif.sendText(msg.to,"SELF PROTECT TAG OFF ✔")
                else:
                    wait["kickMention"] = False
                    if wait["lang"] == "JP":
                        arif.sendText(msg.to,"already turn OF")
...
#-----------------------------------------------
                ...
                if msg.to in wait2['readPoint']:
                        ...
                        with open('sider.json', 'w') as fp:
                         ...
                         arif.sendText(msg.to,"Lurarifng already on")
                else:
                    try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
                ...
                if msg.to not in wait2['readPoint']:
                    arif.sendText(msg.to,"Lurarifng already off")
                else:
                    try:
                            del wait2['readPoint'][msg.to]
                            del wait2['readMember'][msg.to]
                    ...
                    if msg.to in wait2['readPoint']:
                        ...
                        arif.sendText(msg.to, "Lurarifng has not been set.")           
 ...
 #---------Fungsi Banlist With Tag--------#
                ...
                if wait["blacklist"] == {}:
                    ...
                    arif.sendText(msg.to,"Blacklisted user")
                ...
                if msg.toType == 2:
                   ...
                   arif.sendText(msg.to,"done ✔")
                   arif.sendText(msg.to,"done ✔")
                   arif.sendText(msg.to,"blacklist done all removed 👮")
                ...
                if msg.toType == 2:
                    ...
                    if matched_list == []:
                        arif.sendText(msg.to,"Daftar hitam pengguna tidak memiliarif")
                        return
...
#------------------------------------------------
                ...
                rnd = ["Iya","Tidak","Bisa Jadi","Mungarifn","Piarifr Sendiri Coy","Meneketehe","Iya Kayaknya","Maebi Yes Maebi No"]
                p = random.choice(rnd)
                lang = 'id'
